...
Node Class
	...
	Variables
		...
		node cost (f(n) estimated cost to get to the goal from the node)
		puzzle state
...
Most of the variables in Node class are required for recovering the path to the solution rather then finding it.
...
It's clear from this example that A-star can be loosely thought of as a form of greedy search where the searches greed is directed by the chosen heuristic. In fact if the heuristic is well chosen A-star provides guarantees generally not available to greedy search algorithms. For instance a heuristic h(n) is consistent if for every node n and any successor n' which can be
arrived at via the search graph

h(n) <= c(n,n') + h(n')

where c(n,n') is the cost of going from node n to node n'. If h(n) is consistent it can be show that h(n) is also admissible, meaning that it never over estimates to cost of getting to the goal node and that its solution optimal, yielding the lowest cost path to the solution state.

One trick I employ in the implementation of this algorithm is to notice that if the puzzle state of node n is,
